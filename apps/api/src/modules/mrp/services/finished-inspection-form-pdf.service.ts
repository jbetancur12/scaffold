...
const finishedInspectionPdfTemplate = `
doctype html
html(lang="es")
  head
    meta(charset="UTF-8")
    meta(name="viewport" content="width=device-width, initial-scale=1.0")
    title= docCode + ' - Inspección Producto Terminado'
    style.
    ...
    .section
      ...
      .grid-2
        ...
        .field
          ...
          .v= verifierName
        .field
          .k Cantidad inspeccionada
          .v= quantityInspected
        .field
          .k Cantidad aprobada / rechazada
          .v= quantityApproved + ' / ' + quantityRejected
    .section
      h3 CHECKLIST INSPECCIÓN FINAL
      table.chk
        thead
          tr
            th Criterio
        ...
        tbody
          tr
            td Talla / dimensiones correctas
            td(class=sizeCheckClass)= sizeCheck
          tr
            td Costuras / integridad del producto
            td(class=stitchingCheckClass)= stitchingCheck
          tr
            td Inspección visual general
            td(class=visualCheckClass)= visualCheck
          tr
            td Etiquetado correcto
            td(class=labelingCheckClass)= labelingCheck
          tr
            td Corresponde al pedido/producto esperado
            td(class=productMatchesOrderClass)= productMatchesOrder
    if observations
      .section
        h3 Observaciones
        .field
...
export class FinishedInspectionFormPdfService {
  ...
  async generateFinishedInspectionFormPdf(batchId: string): Promise<{ fileName: string; buffer: Buffer }> {
    const batch = await this.batchRepo.findOne(
      { id: batchId },
      { populate: ['productionOrder', 'variant', 'variant.product'] }
    );
    if (!batch) throw new AppError('Lote no encontrado', 404);
    if (!batch.finishedInspectionFormCompleted || !batch.finishedInspectionFormData) {
      throw new AppError('El lote no tiene FOR de inspección final diligenciado', 400);
    }

    const data = (batch.finishedInspectionFormData || {}) as Record<string, unknown>;
    const docCode = batch.finishedInspectionFormDocumentCode || 'N/A';
    const docVersion = Number(batch.finishedInspectionFormDocumentVersion || 1);
    const docDate = this.formatDate(batch.finishedInspectionFormDocumentDate);
    const docTitle = batch.finishedInspectionFormDocumentTitle || 'Inspección de Producto Terminado';
    ...
    const locals = {
      ...
      inspectorName: String(data.inspectorName || batch.finishedInspectionFormFilledBy || 'N/A'),
      verifierName: String(data.verifierName || 'N/A'),
      quantityInspected: Number(data.quantityInspected || 0),
      quantityApproved: Number(data.quantityApproved || 0),
      quantityRejected: Number(data.quantityRejected || 0),
      sizeCheck: data.sizeCheck ? 'Sí' : 'No',
      sizeCheckClass: data.sizeCheck ? 'yes' : 'no',
      stitchingCheck: data.stitchingCheck ? 'Sí' : 'No',
      stitchingCheckClass: data.stitchingCheck ? 'yes' : 'no',
      visualCheck: data.visualCheck ? 'Sí' : 'No',
      visualCheckClass: data.visualCheck ? 'yes' : 'no',
      labelingCheck: data.labelingCheck ? 'Sí' : 'No',
      labelingCheckClass: data.labelingCheck ? 'yes' : 'no',
      productMatchesOrder: data.productMatchesOrder ? 'Sí' : 'No',
      productMatchesOrderClass: data.productMatchesOrder ? 'yes' : 'no',
      ...
      formFilledBy: batch.finishedInspectionFormFilledBy || 'N/A',
    };

    const pug = this.loadPug();
    const playwright = this.loadPlaywright();
    const html = pug.compile(finishedInspectionPdfTemplate, { pretty: false })(locals);
    ...
    try {
      ...
      const fileName = `FOR-INSPECCION-PT-${batch.code}.pdf`;
