...
export class QualityIncomingService {
    ...
    private normalizeLotToken(value: string | undefined, fallback: string): string {
    ...
    private formatDateToken(date = new Date()): string {
    ...
    private async generateInternalSupplierLotCode(row: IncomingInspection, warehouse: Warehouse): Promise<string> {
        const materialToken = this.normalizeLotToken(row.rawMaterial?.sku, 'MP');
        const dateToken = this.formatDateToken();
        const prefix = `INT-${materialToken}-${dateToken}-`;

        const existingLots = await this.rawMaterialLotRepo.find({
            rawMaterial: row.rawMaterial.id,
            warehouse: warehouse.id,
            supplierLotCode: { $ilike: `${prefix}%` },
        });

        let maxSeq = 0;
        for (const lot of existingLots) {
            const code = String(lot.supplierLotCode || '');
        ...
        return `${prefix}${nextSeq}`.slice(0, 120);
    }
    ...
    async resolveIncomingInspection(id: string, payload: {
        ...
        const requestedSupplierLotCode = payload.supplierLotCode?.trim() || undefined;
        row.supplierLotCode = requestedSupplierLotCode;
        ...
        if (!isConditional && payload.quantityAccepted > 0) {
            ...
            let supplierLotCode = (requestedSupplierLotCode || row.supplierLotCode || '').trim();
            const lotWasAutoGenerated = !supplierLotCode;
            if (!supplierLotCode) {
                supplierLotCode = await this.generateInternalSupplierLotCode(row, releasedWarehouse);
            }
            ...
            if (!lot) {
                lot = this.rawMaterialLotRepo.create({
                    ...
                    receivedAt: new Date(),
                    notes: lotWasAutoGenerated
                        ? `Entrada por inspección ${row.id} (lote interno autogenerado)`
                        : `Entrada por inspección ${row.id}`,
            ...
            const kardex = this.rawMaterialKardexRepo.create({
                ...
                referenceId: row.id,
                notes: lotWasAutoGenerated
                    ? `Ingreso MP por inspección aprobada (lote interno ${supplierLotCode})`
                    : `Ingreso MP por inspección aprobada (${supplierLotCode})`,
                occurredAt: new Date(),
        ...
        await this.logEvent({
            ...
            metadata: {
                ...
                autoGeneratedSupplierLotCode: Number(row.quantityAccepted) > 0 && !requestedSupplierLotCode,
                supplierLotCode: row.supplierLotCode,
            },
        });
        return row;
    }
