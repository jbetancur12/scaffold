import { NonConformityStatus, QualitySeverity } from '@scaffold/types';
                        ...
                        <CardContent>
                            <form className="grid grid-cols-1 md:grid-cols-2 gap-3" onSubmit={model.handleCreateNc}>
                                <div className="space-y-1">
                                    <Label>Título</Label>
                                    <Input value={model.ncForm.title} onChange={(e) => model.setNcForm((p) => ({ ...p, title: e.target.value }))} required />
                                </div>
                                <div className="space-y-1">
                                ...
                                <div className="space-y-1 md:col-span-2">
                                    ...
                                    <Textarea value={model.ncForm.description} onChange={(e) => model.setNcForm((p) => ({ ...p, description: e.target.value }))} required />
                                </div>
                                <div className="space-y-1">
                                    <Label>Origen</Label>
                                ...
                                <div className="flex items-end justify-end">
                                    <Button type="submit" disabled={model.creatingNc}>{model.creatingNc ? 'Guardando...' : 'Crear No Conformidad'}</Button>
                                </div>
                            </form>
                        </CardContent>
                    </Card>
                            ...
                            {model.loadingNc ? <div>Cargando...</div> : model.nonConformities.length === 0 ? <div className="text-sm text-slate-500">Sin registros.</div> : model.nonConformities.map((n) => (
                                <div key={n.id} className="border rounded-md p-3 flex items-start justify-between gap-4">
                                    ...
                                    {n.status !== NonConformityStatus.CERRADA ? (
                                        <Button size="sm" variant="outline" onClick={() => model.quickCloseNc(n.id)}>Cerrar</Button>
                                    ) : null}
                                </div>
                            ))}
                        </CardContent>
